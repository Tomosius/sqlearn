...
| 133 | Read-only data safety guarantee: Backend protocol contains ONLY read operations (SELECT, DESCRIBE). No INSERT, UPDATE, DELETE, ALTER TABLE, DROP. All intermediate state (fold columns, layer materialization) uses session-scoped temp views that disappear when connection closes. | Source data must never be modified — not by accident, not by design. Users trust sqlearn with production databases. One accidental ALTER TABLE = lost trust forever. Temp views give all the materialization benefits with zero mutation risk. Enforced at the protocol level. |
| 134 | Compile Once, Execute Many: Pipeline compiles a frozen `QueryTemplate` (parameterized AST with `:k` fold slot) once. CV system binds per-fold values and executes. No fold-specific state stored on the pipeline. | Re-compiling the same AST K times per CV is wasted work. Template approach: 1 compilation + K bind-and-execute. Pipeline is the compiler, CV system is the executor. Clean separation of concerns. |
| 135 | Zero-variance column skipping at output boundary: superset discovery produces all possible columns (consistent schema across folds), constant columns detected via DuckDB zone maps (`MIN=MAX`, O(1)) and excluded at numpy handoff before model training. | OHE superset guarantees consistent schema, but produces all-zero columns in folds missing a category. Model shouldn't train on zero-information features. Detection is free (zone maps). Exclusion at output boundary preserves SQL schema consistency while giving the model only informative features. |
