...
class Transformer:
    ...
    def get_step_sql(self, name):
        """Inspect the SQL generated by a specific named step.
        Useful for debugging and auditing pipeline behavior."""
        ...

    def __sklearn_is_fitted__(self):
        """sklearn compatibility: enables sklearn's check_is_fitted() utility."""
        return self._fitted
...
- **Immutability:** Transformers must not mutate incoming expressions. `expressions()`
  receives `exprs` dict — return NEW expressions, don't modify input AST nodes in place.
- **Connection injection:** Transformers don't own their DuckDB connection. The pipeline
  injects it at fit/transform time. Keeps transformers decoupled from execution engine.
- **Learned attributes end in `_`:** Following sklearn convention, fitted state uses
  trailing underscore: `self.params_`, `self.sets_`, `self.columns_`, `self.input_schema_`.
- **Lazy SQL building:** `fit()` collects aggregation expressions, `transform()` emits
  sqlglot ASTs. No SQL is executed until a terminal call (`transform()`, `to_sql()`).
